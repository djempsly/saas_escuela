/**
 * Template de Boletín para Politécnico - República Dominicana
 * Implementa el formato MINERD con 7 competencias fundamentales
 * y módulos técnicos con resultados de aprendizaje
 */
...
/**
 * Template para boletines de Politécnico (MINERD - RD)
 */
export class PolitecnicoDoTemplate extends BaseBoletinTemplate {
  ...
  getMetadata(): TemplateMetadata {
    return {
      sistemaEducativo: 'POLITECNICO_DO',
      nombre: 'Boletín Politécnico MINERD',
      descripcion: 'Formato oficial del Ministerio de Educación para instituciones politécnicas',
  ...
  validateData(): { valid: boolean; errors: string[] } {
    const baseValidation = super.validateData();

    // Validaciones adicionales para politécnico
    if (!this.data.competencias || this.data.competencias.length === 0) {
      baseValidation.errors.push('Se requieren competencias para el boletín politécnico');
    }
  ...
  // Calcular promedio de período considerando recuperación
  private calcularPromedioPeriodo(
    competencias: Competencia[],
    periodo: 'p1' | 'p2' | 'p3' | 'p4',
    recuperacion: 'rp1' | 'rp2' | 'rp3' | 'rp4'
  ): number {
    const notas = competencias.map((c) => {
      const notaPeriodo = c.calificaciones[periodo];
      const notaRecuperacion = c.calificaciones[recuperacion];
      // Si tiene recuperación y es mayor, usar esa (máximo 70)
      if (notaRecuperacion && notaPeriodo && notaRecuperacion > notaPeriodo) {
        return Math.min(notaRecuperacion, 70);
      }
  ...
  render(): React.ReactElement {
    ...
    const gradoColor = this.getGradoColor(estudiante.grado);

    // Calcular promedios por período
    const promedioP1 = this.calcularPromedioPeriodo(competencias, 'p1', 'rp1');
    const promedioP2 = this.calcularPromedioPeriodo(competencias, 'p2', 'rp2');
    const promedioP3 = this.calcularPromedioPeriodo(competencias, 'p3', 'rp3');
    const promedioP4 = this.calcularPromedioPeriodo(competencias, 'p4', 'rp4');
    const calificacionFinal = (promedioP1 + promedioP2 + promedioP3 + promedioP4) / 4;

    const styles = createStyles(gradoColor);
    ...
    const NotaCell = ({ valor, width = '6.25%' }: { valor?: number; width?: string }) => (
      <View style={[styles.tableCell, { width }]}>
        <Text style={[styles.notaCell, { color: this.getNotaColor(valor) }]}>
    ...
    return (
      <Document>
        ...
        <Page size={[LEGAL_WIDTH, LEGAL_HEIGHT]} style={styles.page}>
          ...
          <View style={styles.header}>
            ...
            <View style={{ textAlign: 'right' }}>
              ...
              <Text style={{ fontSize: 7, color: '#6b7280' }}>Año Escolar: {ciclo.añoEscolar}</Text>
              <Text style={{ fontSize: 7, color: '#6b7280' }}>Fecha: {fechaEmision}</Text>
            </View>
          </View>
          ...
          <View style={styles.section}>
            ...
            <View style={{ flexDirection: 'row', justifyContent: 'space-around', padding: 10, backgroundColor: '#f8fafc', borderRadius: 4 }}>
              <View style={{ textAlign: 'center' }}>
                <Text style={{ fontSize: 16, fontWeight: 'bold', color: this.getNotaColor(promedioP1) }}>
                  {promedioP1.toFixed(1)}
                </Text>
                <Text style={{ fontSize: 7, color: '#6b7280' }}>Prom. P1</Text>
              </View>
              <View style={{ textAlign: 'center' }}>
                <Text style={{ fontSize: 16, fontWeight: 'bold', color: this.getNotaColor(promedioP2) }}>
                  {promedioP2.toFixed(1)}
                </Text>
                <Text style={{ fontSize: 7, color: '#6b7280' }}>Prom. P2</Text>
              </View>
              <View style={{ textAlign: 'center' }}>
                <Text style={{ fontSize: 16, fontWeight: 'bold', color: this.getNotaColor(promedioP3) }}>
                  {promedioP3.toFixed(1)}
                </Text>
                <Text style={{ fontSize: 7, color: '#6b7280' }}>Prom. P3</Text>
              </View>
              <View style={{ textAlign: 'center' }}>
                <Text style={{ fontSize: 16, fontWeight: 'bold', color: this.getNotaColor(promedioP4) }}>
          ...
          <View
            style={[
              styles.situacionBox,
              situacionFinal === 'PROMOVIDO' || situacionFinal === 'APROBADO'
                ? styles.promovido
        ...
        <Page size={[LEGAL_WIDTH, LEGAL_HEIGHT]} style={styles.page} orientation="landscape">
          ...
          <View style={[styles.header, { marginBottom: 8 }]}>
            <View style={styles.headerInfo}>
              ...
              <Text style={{ fontSize: 6, color: '#6b7280' }}>
                {this.getNombreCompleto()} | {estudiante.grado} - {estudiante.seccion} | {ciclo.añoEscolar}
              </Text>
            </View>
          </View>
          ...
          <View style={styles.section}>
            <Text style={[styles.sectionTitle, { fontSize: 7 }]}>COMPETENCIAS ESPECÍFICAS ({competencias.length})</Text>
            <View style={styles.table}>
              ...
              {competencias.map((comp, idx) => {
                const promComp = (
                  (comp.calificaciones.p1 || 0) +
                  (comp.calificaciones.p2 || 0) +
                  (comp.calificaciones.p3 || 0) +
                  (comp.calificaciones.p4 || 0)
                ) / 4;
                return (
                  <View key={comp.codigo} style={[styles.tableRow, idx % 2 === 1 ? styles.tableRowAlt : {}]}>
                    ...
                    <NotaCell valor={comp.calificaciones.p1} />
                    <NotaCell valor={comp.calificaciones.rp1} />
                    <NotaCell valor={comp.calificaciones.p2} />
                    <NotaCell valor={comp.calificaciones.rp2} />
                    <NotaCell valor={comp.calificaciones.p3} />
                    <NotaCell valor={comp.calificaciones.rp3} />
                    <NotaCell valor={comp.calificaciones.p4} />
                    <NotaCell valor={comp.calificaciones.rp4} />
                    <View style={[styles.tableCell, styles.tableCellPromedio]}>
                      <Text style={[styles.notaCell, { color: this.getNotaColor(promComp) }]}>
                        {promComp.toFixed(1)}
                      </Text>
                    </View>
                  </View>
              ...
              <View style={styles.promediosRow}>
                ...
                <View style={[styles.tableCell, styles.tableCellNota]}>
                  <Text style={[styles.notaCell, { color: this.getNotaColor(promedioP1) }]}>
                    {promedioP1.toFixed(1)}
                  </Text>
                </View>
                <View style={[styles.tableCell, styles.tableCellNota]}><Text>-</Text></View>
                <View style={[styles.tableCell, styles.tableCellNota]}>
                  <Text style={[styles.notaCell, { color: this.getNotaColor(promedioP2) }]}>
                    {promedioP2.toFixed(1)}
                  </Text>
                </View>
                <View style={[styles.tableCell, styles.tableCellNota]}><Text>-</Text></View>
                <View style={[styles.tableCell, styles.tableCellNota]}>
                  <Text style={[styles.notaCell, { color: this.getNotaColor(promedioP3) }]}>
                    {promedioP3.toFixed(1)}
                  </Text>
                </View>
                <View style={[styles.tableCell, styles.tableCellNota]}><Text>-</Text></View>
                <View style={[styles.tableCell, styles.tableCellNota]}>
                  <Text style={[styles.notaCell, { color: this.getNotaColor(promedioP4) }]}>
                    {promedioP4.toFixed(1)}
                  </Text>
                </View>
                <View style={[styles.tableCell, styles.tableCellNota]}><Text>-</Text></View>
                ...
                <View key={modulo.codigo} style={{ marginBottom: 8 }}>
                  ...
                  <View style={styles.table}>
                    ...
                    {modulo.resultadosAprendizaje.map((ra, idx) => {
                      const promRA = (
                        (ra.calificaciones.p1 || 0) +
                        (ra.calificaciones.p2 || 0) +
                        (ra.calificaciones.p3 || 0) +
                        (ra.calificaciones.p4 || 0)
                      ) / 4;
                      const estadoColor = ra.estado === 'LOGRADO' ? '#22c55e' : ra.estado === 'EN_PROCESO' ? '#f59e0b' : '#ef4444';
                      return (
                        <View key={ra.codigo} style={[styles.tableRow, idx % 2 === 1 ? styles.tableRowAlt : {}]}>
                          ...
                          <NotaCell valor={ra.calificaciones.p1} />
                          <NotaCell valor={ra.calificaciones.rp1} />
                          <NotaCell valor={ra.calificaciones.p2} />
                          <NotaCell valor={ra.calificaciones.rp2} />
                          <NotaCell valor={ra.calificaciones.p3} />
                          <NotaCell valor={ra.calificaciones.rp3} />
                          <NotaCell valor={ra.calificaciones.p4} />
                          <NotaCell valor={ra.calificaciones.rp4} />
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginTop: 10, padding: 5, backgroundColor: '#f8fafc', borderRadius: 4 }}>
            ...
            <View style={{ flexDirection: 'row', gap: 8 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
                <View style={{ width: 8, height: 8, backgroundColor: this.config.colorNotas.excelente, borderRadius: 2 }} />
                <Text style={{ fontSize: 5 }}>≥90</Text>
              </View>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
                <View style={{ width: 8, height: 8, backgroundColor: this.config.colorNotas.bueno, borderRadius: 2 }} />
                <Text style={{ fontSize: 5 }}>80-89</Text>
              </View>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
                <View style={{ width: 8, height: 8, backgroundColor: this.config.colorNotas.regular, borderRadius: 2 }} />
                <Text style={{ fontSize: 5 }}>70-79</Text>
              </View>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 2 }}>
                <View style={{ width: 8, height: 8, backgroundColor: this.config.colorNotas.deficiente, borderRadius: 2 }} />
...
// Función para crear estilos dinámicos basados en el color del grado
function createStyles(gradoColor: string) {
...
registerTemplate('POLITECNICO_DO', PolitecnicoDoTemplate);

export default PolitecnicoDoTemplate;
