  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (urlImages.length > 0) {
        formData.append('fotosUrls', JSON.stringify(urlImages));
      }
      ...
      await actividadesApi.create(formData);

      setMessage({ type: 'success', text: 'Actividad creada correctamente' });

      // Limpiar URLs de objetos
      imagenes.forEach((img) => {
        if (!img.isUrl && img.url.startsWith('blob:')) {
      ...
      setMessage({
        type: 'error',
        text: apiError.response?.data?.message || 'Error al crear la actividad',
      });
    } finally {
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Crea una nueva actividad para el landing page
          </p>
        </div>
      </div>
              ...
              <div className="space-y-4">
                ...
                <div className="relative aspect-video bg-slate-100 rounded-lg overflow-hidden">
                  <Image
                    ...
                    unoptimized={imagenes[currentSlide]?.isUrl}
                  />
